...
|---|---|---|---|
| `entity` | 自定义业务对象 | 对象定义（Account、Contact 等） | 16 |
...
`db_table` 决定了该元模型的数据实例存储在哪张物理表中，是数据路由的核心依据。老系统通过 `db_table` 前缀判断存储模式：

| db_table 值 | 存储模式 | 说明 |
|---|---|---|
| `p_meta_metamodel_data` | Common 大宽表模式 | 数据存储在 `p_common_metadata`（Common 级）和 `p_tenant_metadata`（Tenant 级），通过 `metamodel_api_key` 区分类型，字段语义由 `p_meta_item.db_column` 定义 |
| `p_custom_entity` 等独立表名 | 独立快捷表模式（老系统） | 数据存储在独立的业务表中，字段语义由表结构直接定义。老系统的 entity/item/entityLink/checkRule/pickOption 使用此模式 |
| `p_meta_tenant_metadata` | Tenant JSON 模式 | 数据以 JSON 格式存储在 Tenant 级大宽表中 |

**新系统的 db_table 统一规则：**

新系统中，所有 6 种实体相关元模型统一使用大宽表模式，`db_table` 应设置为 `p_meta_metamodel_data`：

| 元模型 | 老系统 db_table | 新系统 db_table | 实际存储表 |
|---|---|---|---|
| entity | p_custom_entity | p_meta_metamodel_data | p_common_metadata + p_tenant_metadata |
| item | p_custom_item | p_meta_metamodel_data | p_common_metadata + p_tenant_metadata |
| entityLink | p_custom_entity_link | p_meta_metamodel_data | p_common_metadata + p_tenant_metadata |
| checkRule | p_custom_check_rule | p_meta_metamodel_data | p_common_metadata + p_tenant_metadata |
| pickOption | p_custom_pickoption | p_meta_metamodel_data | p_common_metadata + p_tenant_metadata |
| referenceFilter | p_custom_refer_filter | p_meta_metamodel_data | p_common_metadata + p_tenant_metadata |

### 3.2 p_meta_item — 元模型字段定义表

定义每种元模型有哪些属性字段。这是大宽表机制的核心——通过 `db_column` 字段将 Java 属性名映射到大宽表的 `dbc_xxxN` 物理列。

**当前数据：175 个字段定义，覆盖 6 种实体相关元模型。**
...
**p_meta_item 中的 db_column 映射到大宽表列的类型分布（6 种实体元模型共 175 个字段）：**
...
| optionApiKey | api_key | 选项apiKey | 文本 |
| optionLabel | option_label | 选项名 | 文本 |
| optionLabelKey | option_label_key | 选项名i18n Key | 文本 |
